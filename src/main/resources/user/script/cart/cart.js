...
function deleteRecordConfirmed(id_product) {
    ...
    $.ajax({
        url: "/ryxen-shop/Cart/DeleteCart?id_product=" + id_product,
        type: "post",
        data: {},
        dataType: "json",
...
function alterProductOrder(status, maxOrder, id) {
    ...
    var oldMoney = parseFloat($('#totalPay').text().trim().replace(/([,.€])+/g, '').split(' ')[0]);
    //var price = parseFloat($('#' + id).find('#price').text().trim().replace(/([,.€])+/g, '').split(' ')[0]);
    var price =  parseInt($('#' + id).find('#price #priceHidden').val());
...
function deleteAllRecordConfirmed() {
    ...
    $.ajax({
        url: "/ryxen-shop/Cart/DeleteSelectedCart?id_product=" + arrIdProductStr,
        type: "POST",
        dataType: 'json',
        data: {},
        contentType: "application/json",
...
function addProductToCartSilent(id_product, amount) {
    ...
    $.ajax({
        url: "/ryxen-shop/cart/add-product",
        type: "post",
        data: JSON.stringify(data),
        dataType: "json",
        contentType: "application/json",
...
function payNow() {
    ...
    if (arrIdProduct.length > 0) {
        ...
        window.location.href = '/ryxen-shop/bill-cart?strIdProduct=' + listIdProduct;
    } else {
