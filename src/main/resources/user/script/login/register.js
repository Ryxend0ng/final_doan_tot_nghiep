    ...
    $('#username').focusout(function () {
        if ($(this).val() == "" || $(this).val() == null) {
            ...
            $.ajax({
                ...
                error: function (e) {
                    console.log("ERROR : ",+"usrMsg: "+ e.responseJSON.usrMsg+"|| devMsg: "+e.responseJSON.devMsg);
                }
            });
        }
    });
    ...
    $('#password').focusout(function () {
        if ($(this).val() == "" || $(this).val() == null) {
            ...
            if (!regexPassword.test($('#password').val())) {
                $('#passwordMessage').find('.message-content').text("Mật khẩu có ít nhất 6 ký tự và 1 ký tự đặc biệt!");
                $('#passwordMessage').show();
            } else {
                $('#passwordMessage').hide();
            }
...
function register() {
    if (validationForm()) {
        ...
        $.ajax({
            ...
            timeout: 600000,
            success: function (jsonResult) {
                $("#code-from-server").val(jsonResult);
                console.log("code:"+jsonResult);
            },
            error: function (e) {
                console.log("ERROR :"+ e);
            }
...
function sendCodeConfirm() {
    ...
    if ($("#code-from-server").val() == $("#code-confirm").val()||true) {
        $("#code-confirm").val("");
        ...
        $.ajax({
            type: "POST",
            url: "/ryxen-shop/register-account",
            data: json,
           // processData: false, //prevent jQuery from automatically transforming the data into a query string
            ...
            success: function (jsonResult) {
                if (jsonResult == true) {
                    ...
                    $(location).attr('href', "/ryxen-shop/dang-nhap.html");
                } else {
                    showAlertMessage("Đăng ký tài khoản thất bại!", false);
                    $(location).attr('href', "/ryxen-shop/dang-ky.html");
                }
            },
            error: function (e) {
            console.log(e);
