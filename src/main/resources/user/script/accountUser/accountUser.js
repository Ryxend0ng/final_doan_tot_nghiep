...
$(document).ready(function () {
    ...
    $("#form-infor").submit(function (e) {
        ...
        submitHandler: function (form) {
            ...
            $.post({
                ...
                url: "/ryxen-shop/update-account",
                data: data,
                processData: false, //prevent jQuery from automatically transforming the data into a query string
                contentType: false,
                success: function (data) {
                    showAlertMessage("Đổi thông tin thành công!", true);
                    window.location.href = '/ryxen-shop/my-account.html';
                },
                error: function (e) {
                    console.log("ERROR : ", e);
                }
    ...
    $("#changePasswordForm").submit(function (e) {
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                type: "POST",
                url: "/ryxen-shop/update-password",
                data: json,
                processData: false, //prevent jQuery from automatically transforming the data into a query string
...
function loadSaleOrder(idAccount) {
    idAccountLogin=idAccount;
    $.ajax({
        url: '/ryxen-shop/order-by-account',
        data: {
            idAccount: idAccount
        },
        type: "GET",
        success: function (jsonResult) {
            ...
            $.each(jsonResult.orders, function (index, order) {
                if (order.processingStatus == 1 || order.processingStatus == 2 || order
                    ...
                    html += '<span id="status-order' + order.id +
                        '">' + status + '</span?';
                    html += '</a>';
                    html += '</li>';
                    html += '</ul>';
                        ...
                        html +=
                            ...
                            <div class="sp_bill px-4">
                                ...
                                <div class="d-flex flex-row">
                                    ...
                                    <p>
                                        Giá: ${detail.price.toLocaleString('it-IT', {
                                style: 'currency',
                                currency: 'VND'
                            })}
                                    </p>
...
function cancelOrder(idOrder) {
    var statusOrder = $(`#status-order${idOrder}`).text();
    console.log("statuss"+statusOrder);
    if(statusOrder === "Giao cho đơn vị vận chuyển") {
        $('#btn_request-cancel').attr("onclick", "cancelOrderNotifyConfirmed(" + idOrder + ")");
        $('#modal-request-cancel').modal('show');
    }else{
        cancelOrderNotifyConfirmedStatus(idOrder);
    }
}

function cancelOrderNotifyConfirmedStatus(idOrder) {
    $('#modal-request-cancel').modal('hide');
    $.ajax({
        url: '/ryxen-shop/request-cancel-order-status',
        data: {
            idOrder: idOrder
        },
        type: "GET",
