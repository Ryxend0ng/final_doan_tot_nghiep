	...
	$("#form--upload").validate({
		...
		submitHandler: function (form) {
			...
			$.ajax({
				...
				url: "/ryxen-shop/admin/add-update-category",
				data: data,
				...
				success: function (data) {
					...
					setTimeout(function () {
						$(location).attr('href', "/ryxen-shop/admin/category.html");
					}, 1600);
				},
				error: function (e) {
					console.log("ERROR : ", e);
...
function loadDetailForEdit(idCategory) {
	$.ajax({
		url: "/ryxen-shop/admin/detail-category",
		type: "get",
		...
		success: function (category) {
			...
			$('.img--avatar').css("background-image", "url(/user/upload/" + category.avatar + ")");
