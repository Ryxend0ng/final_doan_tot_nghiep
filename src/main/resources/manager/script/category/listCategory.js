$(document).ready(function () {
	...
	$("body").on("change", ".btnChangeStatus", function (e) {
		...
		$.post({
			url: "/ryxen-shop/admin/change-status-category",
			data: {
				status: status,
				id: id
			},
...
function detail(id) {
	window.location.href = '/ryxen-shop/admin/category-detail/' + $('#view_' + id).val();
}

function edit(id) {
	window.location.href = '/ryxen-shop/admin/edit-category/' + $('#edit_' + id).val();
}
...
function deleteConfirmed(idCategory) {
	...
	$.ajax({
		url: '/ryxen-shop/admin/delete-category/'+idCategory,
		dataType: "json",
		type: "DELETE",
...
function loadCategory(keySearch, currentPage) {
	...
	$.get({
		url: "/ryxen-shop/admin/all-category",
