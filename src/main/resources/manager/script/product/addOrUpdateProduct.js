	...
	$("#formDetailProduct").validate({
		...
		submitHandler: function (form1) {
			...
			$.ajax({
				...
				url: "/ryxen-shop/admin/add-update-product",
				data: data,
				...
				success: function (data) {
					...
					$(location).attr('href', "/ryxen-shop/admin/product.html");
				},
				error: function (e) {
					console.log("ERROR : ", e);
				}
...
function loadDetailForEdit(id_product) {
	$.get({
		url: "/ryxen-shop/product/" + id_product,
		contentType: "application/json", //set data send to server is json
		success: function (result) {
			...
			$('.img--avatar').css("background-image", "url(/user/upload/" + result.avatar + ")");
			...
			$.each(result.productImages, function (index, value) {
				...
				$(".image--product").last().css("background-image", "url(/user/upload/" + value
					.path + value.title + ")");
