$(document).ready(function () {
    ...
    $("body").on("change", ".btnChangeStatus", function (e) {
        ...
        $.post({
            url: "/ryxen-shop/admin/change-detail-product",
            data: {
                status: status,
                id: id,
                type: type
...
function loadCategory() {
    $.get({
        url: "/ryxen-shop/admin/all-category-active",
...
function loadProduct(keySearch, currentPage, idCategory, status) {
    ...
    $.get({
        url: "/ryxen-shop/admin/all-product",
        data: {
                ...
                $.each(result.products, function (index, value) {
                    html += `<tr class="tr-shadow">
                                <td class="block-image">
                                    <img src="/user/upload/${value.avatar}" alt="Hình Ảnh Sản Phẩm" />
                                </td>
                                <td>
                                     ${value.title}
                                </td>
...
function detail(id) {
    window.location.href = '/ryxen-shop/admin/product-detail/' + $('#view_' + id).val();
}

function edit(id) {
    window.location.href = '/ryxen-shop/admin/edit-product/' + $('#edit_' + id).val();
}
...
function deleteConfirmed(idProduct) {
    ...
    $.ajax({
        url: '/ryxen-shop/product/' + idProduct,
        type: "DELETE",
        success: function (result) {
            if (result == true) {
