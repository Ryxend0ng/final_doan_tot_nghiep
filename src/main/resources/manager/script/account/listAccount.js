...
$(document).ready(function () {
    ...
    $("body").on("change", ".btnChangeStatus", function (e) {
        ...
        $.post({
            url: "/ryxen-shop/admin/change-status-account",
            data: {
                status: status,
                id: id
            },
...
function loadStaff(page) {
    ...
    var type = 0;
    $.ajax({
        url: '/ryxen-shop/admin/list-account',
        type: "GET",
        data: {
            type: type,
            page: page
            ...
            $.each(result.listEntity, function (i, item) {
                html += `   <tr class="tr-shadow">
                                ...
                                <td class="block-image">
                                    <img src="/user/upload/${item.avatar!=null ? item.avatar : "noAvatar.png" }" alt="" />
                                </td>
                                <td>
...
function decentralization(idAccount) {
    window.location.href = '/ryxen-shop/admin/decentralization-account/' + idAccount;
}